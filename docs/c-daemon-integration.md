...
1. **Table ownership must be strictly disjoint across daemons.**
   ...
   Concretely: `serverd` must never issue `SELECT`/`INSERT`/`DELETE` against the logger table, and `loggerd` must never touch any table `serverd` owns. If a future feature needs both daemons to touch the same table, this architecture no longer applies — see Section 6.
